...
public class Miningdimension1Teleporter implements ITeleporter {
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("miningdimension_1_portal", Vec3i::compareTo, 300);
	public static PoiType poi = null;

	@SubscribeEvent
	public static void registerPointOfInterest(RegistryEvent.Register<PoiType> event) {
		poi = new PoiType("miningdimension_1_portal",
				Sets.newHashSet(
						ImmutableSet.copyOf(MiningDimensionCaveVersionModBlocks.MININGDIMENSION_1_PORTAL.getStateDefinition().getPossibleStates())),
				0, 1).setRegistryName("miningdimension_1_portal");
	...
	public Miningdimension1Teleporter(ServerLevel worldServer, BlockPos entityEnterPos) {
				...
				for (int j3 = 0; j3 < 2; ++j3) {
					for (int k3 = -1; k3 < 3; ++k3) {
						BlockState blockstate1 = k3 < 0
								? MiningDimensionCaveVersionModBlocks.MINERALWORLDPORTAL.defaultBlockState()
		...
		for (int l1 = -1; l1 < 3; ++l1) {
			for (int j2 = -1; j2 < 4; ++j2) {
				if (l1 == -1 || l1 == 2 || j2 == -1 || j2 == 3) {
					...
					this.level.setBlock(blockpos$mutableblockpos, MiningDimensionCaveVersionModBlocks.MINERALWORLDPORTAL.defaultBlockState(), 3);
				}
			}
		}
	...
	private PortalInfo getPortalInfo(Entity entity, ServerLevel server) {
		...
		return this.getPortalRepositioner(entity, blockpos1).map(repositioner -> {
			...
			if (blockstate.hasProperty(BlockStateProperties.HORIZONTAL_AXIS)) {
				...
				vector3d = Miningdimension1PortalShape.getRelativePosition(teleportationrepositioner$result, direction$axis, entity.position(),
						entity.getDimensions(entity.getPose()));
			} else {
				direction$axis = Direction.Axis.X;
				vector3d = new Vec3(0.5, 0, 0);
			}
			return Miningdimension1PortalShape.createPortalInfo(server, repositioner, direction$axis, vector3d,
